...
router.post('/warehouses/:id/staff', async (req:any, res:any) => {
  try {
...
router.get('/warehouses/:id/staff', async (req:any, res:any) => {
  try {
    const warehouse = await Warehouse.findById(req.params.id);
    if (!warehouse) {
      return res.status(404).json({ error: 'Warehouse not found' });
...
router.put('/warehouses/:id/staff/:staffId', async (req:any, res:any) => {
  try {
    const warehouse = await Warehouse.findById(req.params.id);
    if (!warehouse) {
      return res.status(404).json({ error: 'Warehouse not found' });
...
router.delete('/warehouses/:id/staff/:staffId', async (req:any, res:any) => {
  try {
    const warehouse = await Warehouse.findById(req.params.id);
    if (!warehouse) {
      return res.status(404).json({ error: 'Warehouse not found' });
